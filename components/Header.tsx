    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#03032e]/95 backdrop-blur-sm border-b border-[rgba(2,127,255,0.1)]">
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <div className="relative" onMouseEnter={() => setProductsOpen(true)} onMouseLeave={() => setProductsOpen(false)}>
            ...
            {productsOpen && (
              <div className="absolute top-full left-0 mt-1 w-52 bg-[#06063f] border border-[rgba(245,158,11,0.2)] rounded-xl shadow-2xl py-2">
                {productsLinks.map(l => (
                  <Link key={l.href} href={l.href} className="block px-4 py-2.5 text-sm text-[#8b8fa8] hover:text-white hover:bg-[rgba(245,158,11,0.05)] transition-colors">{l.label}</Link>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="relative" onMouseEnter={() => setCompanyOpen(true)} onMouseLeave={() => setCompanyOpen(false)}>
            ...
            {companyOpen && (
              <div className="absolute top-full left-0 mt-1 w-40 bg-[#06063f] border border-[rgba(245,158,11,0.2)] rounded-xl shadow-2xl py-2">
                {companyLinks.map(l => (
                  <Link key={l.href} href={l.href} className="block px-4 py-2.5 text-sm text-[#8b8fa8] hover:text-white hover:bg-[rgba(245,158,11,0.05)] transition-colors">{l.label}</Link>
                ))}
              </div>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-[#06063f] border-t border-[rgba(2,127,255,0.1)] px-6 py-4 space-y-3">
          ...
          <div className="text-xs text-[#f59e0b] uppercase tracking-widest mt-3 mb-1">AI Products</div>
          ...
          <div className="text-xs text-[#f59e0b] uppercase tracking-widest mt-3 mb-1">Company</div>
